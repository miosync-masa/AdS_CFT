# PhaseShift‑IXc — gate_delay=2 (stronger delayed causality)
# -------------------------------------------------------------
# Same settings as IXb but with delayed gating by 2 steps to amplify
# positive‑lag causality from λ_pre to S_RT_mo.
#
# Exports:
#  /mnt/data/metrics_phaseshift9c.csv
#  /mnt/data/crosscorr_out_pre_phaseshift9c.png
#  /mnt/data/transfer_entropy_phaseshift9c.png
#  /mnt/data/summary_report_phaseshift9c.txt

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

ua3 = Automaton(H=44, W=44, Z=24, L_ads=1.0, alpha=0.9,
                gate_delay=2, gate_strength=0.15, c_eff_max=0.18)
ua3.boundary = ua3.coop_field()
...
for t in range(1, T+1):
    ...
    ua3.update_bulk()
    R = ua3.region_A(t)
    Lb_pre = ua3.K_over_V(ua3.boundary)
    ...
    c_eff = float(np.clip(ua3.c0*(1.0 + ua3.gamma*norm_tail), 0.08, ua3.c_eff_max))

    ua3.HR(c_eff)
    applied_px = ua3.apply_pending_gate()
    new_mask = ua3.compute_gate_mask(R, Lb_pre)
    if new_mask is not None:
        ua3.pending_masks[-1] = new_mask

    ua3.update_boundary_payoff()
    ua3.SOC_tune()

    S_mo, parts = ua3.S_RT_multiobjective(R)
    rec = dict(
        t=t,
        entropy_RT_mo=S_mo,
    ...
    rec["dS_RT_mo"] = 0.0 if prev is None else S_mo - prev
    prev = S_mo
    rows.append(rec)

dfc = pd.DataFrame(rows)
csv_path = "/mnt/data/metrics_phaseshift9c.csv"
...
S = dfc["entropy_RT_mo"].values
lam_out_pre = dfc["lambda_p99_A_out_pre"].values
...
def discretize_quantiles(x: np.ndarray, n_bins:int=3):
    ...
    edges = np.quantile(x, qs); edges = np.unique(edges)
    if len(edges)<=2:
...
def transfer_entropy(x: np.ndarray, y: np.ndarray, n_bins:int=3) -> float:
    xq = discretize_quantiles(x, n_bins); yq = discretize_quantiles(y, n_bins)
...
with open("/mnt/data/summary_report_phaseshift9c.txt","w") as f:
    f.write(f"[PRE driver, delay=2] Best Pearson cross-corr: {maxcorr:.3f} at lag {best_lag}\n")
...
plt.title("Cross-correlation S_RT_mo vs λ_p99_A_out_pre — PhaseShift‑IXc (delay=2)")
plt.tight_layout(); plt.savefig("/mnt/data/crosscorr_out_pre_phaseshift9c.png"); plt.close()

plt.figure()
plt.bar([0,1], [TE_L_to_S, TE_S_to_L])
plt.xticks([0,1], ["λ_pre→S", "S→λ_pre"])
plt.ylabel("Transfer Entropy (nats)")
plt.title("Directionality via Transfer Entropy (pre‑λ as driver) — PhaseShift‑IXc")
plt.tight_layout(); plt.savefig("/mnt/data/transfer_entropy_phaseshift9c.png"); plt.close()

print(f"[PRE driver, delay=2] Best Pearson corr = {maxcorr:.3f} at lag {best_lag}")
